...
public class Attendee_NotificationActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        event = (Event) getIntent().getSerializableExtra(EXTRA_STUDENT_CLASS);
        className.setText(event.getName());

        publicPreferences = AndroidUtils.getCommonSharedPrefs(getApplicationContext());
        user = new UserUtils(this).getUserFromSharedPrefs();

        String messagesString = publicPreferences.getString(AppConstants.KEY_LOGGED_IN_USER_NOTIFICATIONS + user.getUserId() + event.getUniqueCode() , null);
    ...
    private void setAdapter() {
        listAdapter = new ListAdapter(Attendee_NotificationActivity.this, messageArrayList);
            ...
            protected void onPostExecute(Void aVoid) {
                if (result != null) {
                    try {
                        ...
                        if (object.has("Error")) {
                            Toast.makeText(Attendee_NotificationActivity.this, object.getString("Error"), Toast.LENGTH_SHORT).show();

                        } else {
                            messageArrayList = DataUtils.getMessagesArrayList(result);
                            publicPreferences.edit().putString(AppConstants.KEY_LOGGED_IN_USER_NOTIFICATIONS + user.getUserId() + event.getUniqueCode(), result).apply();
                            setAdapter();
                        }
